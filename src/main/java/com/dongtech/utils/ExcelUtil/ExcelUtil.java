/**
 *       * @文件名称: ImportExcel.java   * @类路径: utils.excelUtil    @描述: TODO    @作者：Administrator
 *  * @时间：2020年1月13日 上午9:36:27   * @版本：V1.0
 */
...
/**
 *    * @类功能说明：   * @类修改者：    @修改日期：   * @修改说明：   * @公司名称：读取并解析excel  * @作者：Administrator
 *  * @创建时间：2020年1月13日 上午9:36:27   * @版本：V1.0
 */
public class ExcelUtil {
   ...
   * @throws Exception
   */
  public ExcelUtil(InputStream in, String xlsPath) throws Exception {
    if (utils.excelUtil.LinkyStringUtil.isNotEmpty(xlsPath)) {
      // 获取扩展名
   ...
   * @return
   */
  public static boolean isExcel(String xlsPath) {
    if (utils.excelUtil.LinkyStringUtil.isEmpty(xlsPath)) {
      return false;
    }
    return xlsPath.endsWith(".xls") || xlsPath.endsWith(".xlsx");
  }
   ...
   *
   * @param sheetIx 指定 sheet 页，从 0 开始
   * @param start 指定开始行，从 0 开始(第一行为表头)
   ...
   *
   * @param response
   * @param excelName 文件名前缀
   * @throws Exception
   */
  public void exportResponse(HttpServletResponse response, String excelName) throws Exception {
    OutputStream output = response.getOutputStream();
    response.reset();
    String fileName =
        excelName
            + "_"
            + LinkyTimeUtil.getInstance().getCurrentTimeStamp("yyyy-MM-dd_HH-mm-ss_SSS");
    response.setHeader(
        "Content-disposition",
   ...
   * @return
   */
  public boolean isMergedRegion(int sheetIx, int row, int column) {
    Sheet sheet = workbook.getSheetAt(sheetIx);
   ...
   * 默认的数据类型：CELL_TYPE_BLANK(3), CELL_TYPE_BOOLEAN(4), CELL_TYPE_ERROR(5),CELL_TYPE_FORMULA(2),
