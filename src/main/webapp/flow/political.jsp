            ...
            <div class="layui-form-item text-right">
                <div style="float:left">
                    ...
                    <div style="margin-left: 20px;">
                        ...
                        <p>
                            <button onclick="Capture()" type="button">政治审查表扫描</button>

                        <p>


                        <ul id="parentUl"></ul>
                        <button id="savezs" type="button">保存提交</button>
                    </div>
                    <input type="hidden" id="subzs" value=""/>
...
//初始富文本编辑器2
        ...
        $("#savezs").click(function (data) {
          ...
          $.post("/flow/addapplication",
              {id: id, nodeId: nodeId, images: path, isgo: true, type: 1},
              function (data) {
                if (data.res) {
        ...
        form.on('submit(pass)', function (data) {
              if ($("#issubmit").val() != 1) {
                layer.msg("请打印政审外调函", {icon: 2});
                return;
              }
              if ($("#subImage").val() != 1) {
                layer.msg("请打印入政治审查情况报告", {icon: 2});
                return;
              }
              if ($("#subzs").val() != 1) {
                layer.msg("请扫描政治审查表", {icon: 2});
                return;
              }
              layer.load(2);
